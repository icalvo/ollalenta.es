...
if (!isset($_POST['s'])) {
    ...
    $messages = Array();
    // basic validation
    if (trim(@$_POST['name']) == "") $messages[] = "Introduce tu nombre";
    if (trim(@$_POST['email']) == "") $messages[] = "Introduce una dirección de correo electrónico correcta";
    if (trim(@$_POST['message']) == "") $messages[] = "Introduce un texto para el mensaje";
    if (trim(@$_POST['humanornot']) == "") $messages[] = "Debes demostrar que no eres un robot";
    if (trim(@$_POST['s']) == "") $messages[] = "Ha habido un error.";
